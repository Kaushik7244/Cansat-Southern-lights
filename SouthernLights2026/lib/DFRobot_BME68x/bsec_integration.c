...
/**
 * @fn bme68x_bsec_update_subscription
 ...
 */
return_values_init bsec_iot_init(float sample_rate, float temperature_offset, bme68x_com_fptr_t bus_write, 
                    bme68x_com_fptr_t bus_read, sleep_fct sleep, uint8_t addr, enum bme68x_intf intf)
{
...
/**
 * @fn bme68x_bsec_trigger_measurement
 * @brief Trigger the measurement based on sensor settings
 *
 * @param sensor_settings     settings of the BME68X sensor adopted by sensor control function
 * @param sleep               pointer to the system specific sleep function
 *
 * @return   uint32_t
 */
static uint32_t bme68x_bsec_trigger_measurement(bsec_bme_settings_t *sensor_settings, sleep_fct sleep)
{
    ...
    if (sensor_settings->trigger_measurement)
    {
        ...
        bme68x_g.tph_sett.os_hum  = sensor_settings->humidity_oversampling;
        bme68x_g.tph_sett.os_pres = sensor_settings->pressure_oversampling;
        bme68x_g.tph_sett.os_temp = sensor_settings->temperature_oversampling;
		bme68x_g.gas_sett.run_gas = sensor_settings->run_gas;
		bme68x_g.gas_sett.heatr_temp = sensor_settings->heater_temperature; /* degree Celsius */
		bme68x_g.gas_sett.heatr_dur  = sensor_settings->heating_duration; /* milliseconds */
		
		/* Select the power mode */
		/* Must be set before writing the sensor configuration */
		bme68x_g.power_mode = BME68X_FORCED_MODE;
		/* Set the required sensor settings needed */
		set_required_settings = BME68X_OST_SEL | BME68X_OSP_SEL | BME68X_OSH_SEL | BME68X_GAS_SENSOR_SEL;
		
		/* Set the desired sensor configuration */
        bme68x_status = bme68x_set_sensor_settings(set_required_settings, &bme68x_g);
             
        /* Set power mode as forced mode and trigger forced mode measurement */
        bme68x_status = bme68x_set_sensor_mode(&bme68x_g);
        
		/* Get the total measurement duration so as to sleep or wait till the measurement is complete */
		bme68x_get_profile_dur(&meas_period, &bme68x_g);
 ...
 * @return      none
 */
static void bme68x_bsec_read_data(int64_t time_stamp_trigger, bsec_input_t *inputs, uint8_t *num_bsec_inputs, int32_t bsec_process_data)
{
    ...
    {
        bme68x_status = bme68x_get_sensor_data(&data, &bme68x_g);
...
int8_t bsec_iot_loop(sleep_fct sleep, get_timestamp_us_fct get_timestamp_us, output_ready_fct output_ready)
{
    ...
    if(waitTime == 0) {
      ...
      lastTime = millis();
      waitTime = bme68x_bsec_trigger_measurement(&sensor_settings, sleep);
      return 1;
    } else if((millis() - lastTime) >= waitTime) {
      waitTime = 0;
      /* Call the API to get current operation mode of the sensor */
      bme68x_get_sensor_mode(&bme68x_g);
      ...
      while (bme68x_g.power_mode == BME68X_FORCED_MODE)
      {
          /* sleep for 5 ms */
          sleep(5);
          bme68x_get_sensor_mode(&bme68x_g);
