...
typedef enum
{
    ...
    The BME68x is factory trimmed, thus the temperature sensor of the BME68x is very accurate. 
...
typedef enum
{
    ...
    the relative change in ambient TVOCs detected by BME68x. 
    ...
    BSEC_OUTPUT_RAW_TEMPERATURE = 6,  /**< Temperature sensor signal [degrees Celcius]. Temperature directly measured by BME68x in degree Celcius. 
    This value is cross-influenced by the sensor heating and device specific heating.*/              
    BSEC_OUTPUT_RAW_PRESSURE = 7,  /**< Pressure sensor signal [Pa]. Pressure directly measured by the BME68x in Pa.*/            
    BSEC_OUTPUT_RAW_HUMIDITY = 8,  /**< Relative humidity sensor signal [%] .Relative humidity directly measured by the BME68x in %.
    This value is cross-influenced by the sensor heating and device specific heating. */
    BSEC_OUTPUT_RAW_GAS = 9, /**< Gas sensor signal [Ohm]. 
    Gas resistance measured directly by the BME68x in Ohm.The restistance value changes due to varying VOC 
    ...
    Temperature measured by BME68x which is compensated for the influence of sensor (heater) in degree Celcius. 
    The self heating introduced by the heater is depending on the sensor operation mode and the sensor supply voltage.*/
    BSEC_OUTPUT_SENSOR_HEAT_COMPENSATED_HUMIDITY = 16,   /**<  Sensor heat compensated humidity [%]. Relative measured by BME68x which is compensated for the influence of sensor (heater) in %.
