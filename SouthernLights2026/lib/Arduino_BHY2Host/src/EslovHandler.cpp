...
// Wait for ESLOV INT pin with a 200 ms deadline — prevents M7 loop from blocking
// if Nicla is slow or unresponsive.
#define WAIT_ESLOV_INT(pin) { unsigned long _t = millis(); while(!digitalRead(pin) && (millis()-_t) < 200) {} }
